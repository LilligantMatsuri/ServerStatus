...
Service_Server_Status_server(){
	...
	if ! wget "https://raw.githubusercontent.com/LilligantMatsuri/ServerStatus/master/script/status-server.service" -O /usr/lib/systemd/system/status-server.service; then
...
Service_Server_Status_client(){
	[[ ! -e "/usr/lib/systemd/system" ]] && mkdir /usr/lib/systemd/system
	if ! wget "https://raw.githubusercontent.com/LilligantMatsuri/ServerStatus/master/script/status-client.service" -O /usr/lib/systemd/system/status-client.service; then
...
Update_Shell(){
	sh_new_ver=$(wget -qO- -t1 -T3 "https://raw.githubusercontent.com/LilligantMatsuri/ServerStatus/master/script/status_systemd.sh" | grep 'sh_ver="' | awk -F "=" '{print $NF}' | sed 's/\"//g'|head -1)
	...
	wget "https://raw.githubusercontent.com/LilligantMatsuri/ServerStatus/master/script/status_systemd.sh" -O status.sh && chmod +x status.sh
